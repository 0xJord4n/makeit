...
for (const f of process.argv.slice(2)) {
  ...
  if (!metaMatch) {
    console.log(`FAIL ${f}: no meta block`);
    fail = 1;
    continue;
  }
  try {
    // Parse-only validation: constructing a Function compiles the source but
    // never runs it - nothing from the checked files executes. Inputs are this
    // repo's own workflow scripts (same trust level as this checker).
    ...
    if (/Date\.now|Math\.random|new Date\(\)/.test(body)) {
      console.log(`FAIL ${f}: forbidden Date.now/Math.random/new Date()`);
      fail = 1;
      continue;
    }
    console.log(`OK ${f}`);
