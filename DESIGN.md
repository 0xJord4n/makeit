...
- **Entry triage + lite mode** — one-session projects don't get the full pipeline (pure overhead) but a lite lane (`phases/00-lite.md`): one bundled mini-interview round, direct single-writer build, then a parallel check fan-out and one clean re-check. Rationale: implementation of a single deliverable is sequential, but verification parallelizes at any scale. Upward escape hatch when scope grows mid-build.
